...
if TYPE_CHECKING:
	from tree_sitter import Node, Tree, Language
...
def find_closest_ancestor_of_type(root: Optional[Node], ancestorType) -> Optional[Node]:
    """
...
def find_first_child_of_type(root, childType, level=default_unused) -> Optional[Node]:
    """
...
def find_all_children_of_type(
    root: Optional[Node], childType, level=default_unused
) -> List[Node]:
    """
...
def find_all_children_of_types(
    root: Optional[Node], childTypes: List[str], level=default_unused
) -> List[Node]:
    """
    Return the all children to the node `root` that has type `childType` within a positive depth of `level`.
    The return order is the same as the search order, which is breadth-first, level order.
    ...
    # Note that we start with root.children, not root, since root is not a candidate
    # Also, we do a copy, or else q.pop is equivalent to root.children.pop, which is actually mutating
    q = [c for c in root.named_children]

    while not isLevelUsed or level >= 0:
        for _ in range(len(q)):
            top = q.pop(0)
            if top.type in childTypes:
                res.append(top)
...
def get_all_children_at_level(root: Node, level) -> List[Node]:
    if not root:
        return []
    if level < 0:
...
def group_by_type_capture(root: Optional[Node], queryStr: str, language: Language) -> Dict[str, List[Node]]:
    """
